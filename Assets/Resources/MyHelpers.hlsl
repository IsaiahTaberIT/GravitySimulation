...
float MinimumDistanceOverTimeStep(float2 V1Start, float2 V1End, float2 V2Start, float2 V2End)
{
    return DistanceToLine(float2(0,0), V2Start - V1Start, V2End - V1End);
}

float powSign(float a, float b)
{
    float signp = sign(a);
...
float2 powSign(float2 a, float2 b)
{
    float2 signp = sign(a);
...
float componentMin(float2 value)
{
    return min(value.x, value.y);

}

float componentMax(float2 value)
{
